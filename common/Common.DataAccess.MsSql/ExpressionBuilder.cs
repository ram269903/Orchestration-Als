    ...
    public static class ExpressionBuilder
    {
        ...
        public static string BuildWhereExpression(SearchFilter searchFilter)
        {
            ...
            foreach (var filter in searchFilter.Filters)
            {
                ...
                var value = GetFilterValue(filter.Value, filter.DataType);

                var columnName = filter.PropertyName.Split('.').Length == 1 ? "" : filter.PropertyName.Replace("."+filter.PropertyName.Split(".")[filter.PropertyName.Split('.').Length-1],"")+".";

                if(!string.IsNullOrEmpty(columnName))
                    filter.PropertyName = filter.PropertyName.Replace(columnName , "");
                ...
                {
                    case Operator.Equals:
                        if (filter.DataType != null && filter.DataType.ToLower().Equals("dropdown"))
                        {
                            conditions.Add($"{columnName}[{filter.PropertyName}] = {value}");
                        }
                        else
                        if (filter.CaseSensitive)
                            conditions.Add($"{columnName}[{filter.PropertyName}] = {value}");
                        else
                            conditions.Add($"lower({columnName}[{filter.PropertyName}]) = lower({value})");
                        break;
                    case Operator.GreaterThan:
                        conditions.Add($"{columnName}[{filter.PropertyName}] > {value}");
                        break;
                    ...
                    case Operator.LessThanOrEqual:
                        ...
                        break;
                    case Operator.Contains:
                        if (filter.DataType.ToLower().Equals("daterange"))
                        {
                            string[] inValues = searchStringValue.Split(',');
                            string value1, value2 = string.Empty;
                            value1 = inValues[0].Trim();
                            value2 = inValues[1].Trim();
                            if (value1 == value2)
                                conditions.Add($"({columnName}[{filter.PropertyName}]) LIKE '%{value1}%'");
                            else

                                conditions.Add($"(convert(date,{columnName}{filter.PropertyName}) >=  '{value1}' and  convert(date,{filter.PropertyName}) <= '{value2}')");
                        }
                        else
                        {
                            if (filter.CaseSensitive)
                                conditions.Add($"{columnName}[{filter.PropertyName}] LIKE '%{searchStringValue}%'");
                            else
                                conditions.Add($"lower({columnName}[{filter.PropertyName}]) LIKE '%{searchStringValue?.ToLower()}%'");
                        }
                        break;
                    case Operator.StartsWith:
                        conditions.Add($"lower({columnName}[{filter.PropertyName}]) LIKE '{searchStringValue}%'");
                        break;
                    case Operator.EndsWith:
                        conditions.Add($"lower({columnName}[{filter.PropertyName}]) LIKE '%{searchStringValue}'");
                        break;
                    ...
                    case Operator.IsNull:
                        conditions.Add($"{columnName}[{filter.PropertyName}] IS NULL {value}");
                        break;
        ...
        private static string GetFilterValue(object filterValue, string dataType)
        {
            if (filterValue?.ToString() == "db.null") return null;

            var filterType = filterValue?.GetType().ToString();

            string value;

            if (filterType == "System.String" && dataType == "dropdown")
                value = $"'{filterValue}'";
            else
            if (filterType == "System.String")
                value = $"'{filterValue}'";
            else
            if (filterType == "System.Text.Json.JsonElement" && dataType == "dropdown")
                value = $"'{filterValue}'";
            else if (filterType == "System.DateTime" )
                value = $"'{Convert.ToDateTime(filterValue).ToString("yyyy-MM-dd HH:mm:ss")}'";
            else if (filterType == "System.Text.Json.JsonElement" && dataType == "date")
                value = $"'{filterValue}'";
            else if (filterType == "System.Text.Json.JsonElement" && dataType == "dateTime")
                value = $"'{filterValue}'";
            else
                value = $"{filterValue}";
